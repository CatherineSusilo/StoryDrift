...
class APIService {
    ...
    static let baseURL = "http://localhost:3000" // TODO: Update for production
    ...
    func getChildren(token: String) async throws -> [ChildProfile] {
        let response: APIResponse<[ChildProfile]> = try await request(
            endpoint: "/api/children",
            token: token
        )
        
        guard let children = response.data else {
    ...
    func createChild(profile: ChildProfile, token: String) async throws -> ChildProfile {
        ...
        guard let child = response.data else {
            throw APIError.noData
        }
    ...
    // MARK: - Stories
    func getStories(childId: String, token: String) async throws -> [Story] {
        let response: APIResponse<[Story]> = try await request(
            endpoint: "/api/stories/child/\(childId)",
            token: token
        )
    ...
    func postVitals(vitals: Vitals, token: String) async throws {
        let _: APIResponse<String> = try await request(
            endpoint: "/api/vitals/child/\(vitals.childId)",
            method: "POST",
            body: vitals,
            token: token
        )
    ...
    func getStatistics(childId: String, token: String) async throws -> ChildStatistics {
        let response: APIResponse<ChildStatistics> = try await request(
            endpoint: "/api/statistics/child/\(childId)",
            token: token
        )
    ...
    // MARK: - Themes
    func getThemes(token: String) async throws -> [StoryTheme] {
        let response: APIResponse<[StoryTheme]> = try await request(
            endpoint: "/api/stories/themes",
            token: token
        )
        
        guard let themes = response.data else {
            throw APIError.noData
        }
