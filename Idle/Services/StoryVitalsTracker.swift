...
class StoryVitalsTracker: ObservableObject {
    ...
    /// Tracks whether SDK is currently in the process of starting (prevents race conditions)
    private var sdkStarting = false
    /// Last time the SDK was stopped (prevents rapid restart cycles)
    ...
    func startTracking(storyId: String, childId: String, vitalsManager: VitalsManager, cameraEnabled: Bool = true) {
        guard !isTracking else { 
            print("[StoryVitalsTracker] ⚠️ Already tracking, ignoring start request")
            return 
        }
        ...
        guard !sdkStarting else {
            print("[StoryVitalsTracker] ⚠️ SDK already starting, ignoring duplicate request")
            return
        }
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 2.0) { [weak self] in
            ...
            print("[StoryVitalsTracker] 🔄 MediaPipe graph teardown complete, starting new session")
            ...
            do {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) { [weak self] in
                    ...
                    do {
                        ...
                        self.sdkStarting = false
                        self.statusHint = "Recording start failed: \(error.localizedDescription)"
                    }
                }
            } catch {
                print("[StoryVitalsTracker] ❌ Error starting processing: \(error)")
                self.sdkStarting = false
                self.statusHint = "Processing start failed: \(error.localizedDescription)"
            }
        }
            ...
            do {
                ...
                DispatchQueue.global(qos: .utility).asyncAfter(deadline: .now() + 0.2) {
                    do {
                        ...
                        print("[StoryVitalsTracker] ⚠️ Warning during stop processing: \(error)")
                    }
                }
            } catch {
                print("[StoryVitalsTracker] ⚠️ Warning during stop recording: \(error)")
            }
        }
        sdkStarted = false
#endif
